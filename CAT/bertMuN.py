...
#        print (cossim)
        ...
        gen.append([cossim, sen, tarl[t][1], tokens[tarl[t][1]]])
    if len(lcache) > 4:
        lcache = lcache[1:]    
...
for i in tqdm(range(len(l))):
    ...
    for sen in gen:
        f.write(tar.strip() + "\n")
        f.write(sen[1].strip() + "\n")
        replaced.append(sen[2])
        replaced_tok.append(sen[3])
        mutat_sen.append(sen[1])
        simi.append(sen[0])
        pos_tag.append(nltk.pos_tag([sen[3]])[0][1])
    ...
    # print(replaced)
    # print(replaced_tok)
#     added_to_replace = pd.DataFrame({"index": [i]*len(mutat_sen),"test_sen": mutat_sen, "simi":simi,"token_idx": replaced, "tokens":replaced_tok, "pos": pos_tag})
#     replace = pd.concat([replace, added_to_replace])
# replace.to_csv("replaced2.csv", index=False)
f.close()
fline.close()
print (time.time() - ft)
