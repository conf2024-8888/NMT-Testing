...
def BertM(bert, berttoken, inpori, bertori, mutate_idx):
    ...
    for index in range(len(topkTokens)):
        ...
        if tt.strip() == tokens[mutate_idx].strip():
            continue
        # pos tag
        # if nltk.pos_tag([tt])[0][1][:2] != nltk.pos_tag([tokens[mutate_idx]])[0][1][:2]:
        #     continue
        l = deepcopy(tokens)
        l[mutate_idx] = tt
...
INDEX= 0
def align_index(bertmodel, berttoken, line, bertori, token_index, top_tokens, grad):
    result = []
    history_ind = []
    ...
    for i in range(len(token_index)):
        ...
        if tokens[idx].lower() == top_tokens[i].lower():
            if idx in history_ind:
                continue
            history_ind.append(idx)
    ...
    global INDEX
    added_csv = pd.DataFrame({"index":[INDEX]*len(result),"test_sen": [i[1] for i in result], "simi": [i[0] for i in result],  "token_idx": replaced, "tokens":replaced_tok, "pos": pos_tag, "grad": grad_lst})
    INDEX +=1
...
with open("./en_token_stop.json") as f:
    sent_data = json.load(f)
f = open("./f_en_mu_grad_stop.txt", "w")
fline = open("./f_en_mu_grad_stop.index", "w")
bertmodel, berttoken, bertori = bertInit()

# mad = float(sys.argv[2])
for i in tqdm(range(len(sent_data))):
    line = sent_data[i]["en"]
    index= sent_data[i]["token_index"]
    top_tokens = sent_data[i]["top_tokens"]
    grad = sent_data[i]["grad"]
    #tag = nlp.pos_tag(line)

    tar, gen, added_csv = align_index(bertmodel, berttoken, line, bertori, index, top_tokens, grad)
    #gen = sorted(gen)[::-1]
    count = 0
    for sen in gen:
        f.write(tar.strip() + "\n")
...
# print(f".........compare_replace_{thres}_{mad}.csv generated!" )
f.close()
fline.close()
print (time.time() - ft)
