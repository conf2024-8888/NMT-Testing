...
for i in tqdm(range(len(old_lines))):
    old_line = old_lines[i].strip().split()#old chinese line
    new_line = new_lines[i].strip().split()#new chinese
...
def write_all(lcs, ed, tfidf, bleu, lcs_d, ed_d, tfidf_d, file_name):
    ...
    for site in range(len(l)):
        ...
        #TODO: there is a bug !tfidf to bleu_d
        flag = False
        if bleu_d[site][0] <= 0.85:#ed[site][0] > 0.1 * max(ed[site][1], ed[site][2]):
            flag = True
        flag = str(flag)
        ...
        for k in range(3, 8): #[delta, old_lcs_sim, new_lcs_sim, old_lines[i], en_old_lines[i], new_lines[i], en_new_lines[i], ora_lines[i]]
            f.write(str(t[k].strip()) + "\n")
        f.write("\n")
    sum_list[0] /= len(l)
    sum_list[1] /= len(l)
...
ti = old_ti + new_ti
ti /= 2
print("write to Com_Answer.txt")
f_edit = open("Com_Answer.txt", "w")
for i in range(4):
...
f_edit.write("time: "+str(time.time() - tttt))
f_edit.close()
